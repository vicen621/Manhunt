...
@Register(name = "hunter")
public record HunterCmd(Main plugin) implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command cmd, @NotNull String label, @NotNull String[] args) {
        ...
        if (args.length != 1) {
            StringUtils.sendMessage(sender, "&cUsage: /hunter <player>");
            return true;
        }
        ...
        if (Main.getManager().isRunner(p)) {
            StringUtils.sendMessage(sender, "&cThe player you're trying to choose is a runner, please choose other player!");
            return true;
        }

        if (Main.getManager().isHunter(p)) Main.getManager().removeHunter(p);
        else Main.getManager().addHunter(p);
